...
class Student extends CI_Controller
{
    ...
    function select_result($param1 = ''){
        ...
        if($param1=='view'){ 
            ...
            if($this->form_validation->run()){
                ...
                $student  = $this->session->userdata('student_id');
                $class = $this->input->post('class');
                $session = $this->input->post('session');
                $term = $this->input->post('term');
                $params = array('Session'=>$session, 'Term'=>$term, 'StudentID'=>$student, 'class_id'=>$class);
              //  die(var_dump($params));
                $result_exists = $this->db->get_where('result', $params)->result_array();
                ...
                $session2 = explode("/", $session)[1]; 
                $param =  $student."/".$class."/".$term."/". $session1."/".$session2;
               
                redirect(base_url()."index.php?student/resultsheet/".$param);
            
            }

            else{

                $page_data['message'] = "Result Not Found! Please select valid term, session, and class";
                $page_data['page_name'] = 'select_result';
        $page_data['page_title'] = 'Select Result';
    ...
    function resultsheet($student, $class, $term, $session1, $session2){
